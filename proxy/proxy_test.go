...
func testingTenantIDFromDatabaseForAddr(addr string, validTenant string) func(map[string]string) (string, error) {
...
func assertConnectErr(t *testing.T, prefix, suffix, expErr string) {
...
func TestLongDBName(t *testing.T) {
	...
	assertConnectErr(t, pgurl, "" /* suffix */, "boom")
	require.Equal(t, longDB, m["database"])
}

func TestFailedConnection(t *testing.T) {
	// TODO(tbg): use datadriven for these.
	...
	for _, sslmode := range []string{"require", "prefer"} {
		assertConnectErr(t, u, "defaultdb_29?sslmode="+sslmode, "unable to reach backend SQL server")
	}
	assertConnectErr(t, u, "defaultdb_29?sslmode=verify-ca&sslrootcert=testserver.crt", "unable to reach backend SQL server")
	assertConnectErr(t, u, "defaultdb_29?sslmode=verify-full&sslrootcert=testserver.crt", "unable to reach backend SQL server")

	// Unencrypted connections bounce.
	for _, sslmode := range []string{"disable", "allow"} {
		assertConnectErr(t, u, "defaultdb_29?sslmode="+sslmode, "server requires encryption")
	}

	// TenantID rejected by test hook.
	assertConnectErr(t, u, "defaultdb_28?sslmode=require", "invalid tenantID")

	// No TenantID.
	assertConnectErr(t, u, "defaultdb?sslmode=require", "malformed database name")
}
